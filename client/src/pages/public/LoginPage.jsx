...
export default function LoginPage() {
  ...
  const onSubmit = async (data) => {
    setServerError('');
    try {
      // Phase 2 will replace this with: await apiClient.post('/auth/login', data)
      await new Promise((r) => setTimeout(r, 600));
      login({ name: data.email.split('@')[0], email: data.email }, 'demo-token');
    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-base-950 px-4">
      ...
      >
        ...
        <p className="mt-6 text-center text-sm text-ink-500">
          ...
          <Link to="/register" className="font-medium text-accent-violet hover:underline">
            Sign up
          </Link>
        </p>
      </motion.div>
